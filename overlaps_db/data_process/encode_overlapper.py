...
class EncodeOverlapper:
    ...
    def overlap_filtered_with_roadmap(self, assembly='hg19', method=None, min_overlap=0.1, export_temp=False,
        ...
        overlap_full_detail_df['RO_size'] = overlap_full_detail_df.apply(lambda row: self.compute_size(row, 'RO'), axis=1)
        overlap_full_detail_df['RO_method'] = overlap_full_detail_df.apply(lambda row: self.set_col_value(row, 'RO_name', 'DNase'),
                                                               axis=1)
        overlap_full_detail_df['RO_ovlp_len'] = overlap_full_detail_df.apply(lambda row: self.compute_ovlp_len(row, 'RO'), axis=1)
        overlap_full_detail_df['RO_ovlp_pct'] = overlap_full_detail_df.apply(lambda row: self.compute_ovlp_pct(row, 'RO'), axis=1)
